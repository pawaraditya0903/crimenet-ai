...
| **Biometric Face Verification** | Production anti-spoof biometric auth | Prototype facial verification using Zero-Normalized Cross Correlation (ZNCC) on normalized facial vectors; labeled as demo prototype | `backend/app/security/face_prototype.py` | `tests/unit/test_face_prototype.py` | **PROTOTYPE** |
