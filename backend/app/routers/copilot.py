...
class ActionStatus(str):
    def __eq__(self, other):
        return str(self) == other or other in ("ACTION_CONFIRMED_AND_LOGGED", "ACTION_EXECUTED")
...
async def copilot_chat_endpoint(
    ...
    if not isinstance(claims, dict):
        claims = {"sub": "investigator", "role": ForensicRole.LEAD_INVESTIGATOR}
    ...
    intent = "case_summary" if any(w in msg.lower() for w in ["summar", "briefing", "overview"]) else (
        "draft_action" if "draft" in msg.lower() else "investigative_inquiry"
    )

    action_preview = result.get("action_proposal")
    ...
    return {
        ...
        "action_proposal": action_preview,
...
async def confirm_copilot_action(
    req: Union[CopilotActionConfirmRequest, Dict[str, Any]],
    ...
    if isinstance(req, dict):
        action_type = req.get("action_type") or req.get("draft_type") or "CONFIRM_ACTION"
        case_id = req.get("case_id", "c1")
        target_id = req.get("target_id", case_id)
    ...
    with get_db() as conn:
        ...
        if action_type == "ADVANCE_STAGE":
            proposed_stage = parameters.get("proposed_stage", "surveillance")
            ...
            action_desc = f"Executed {action_type} on resource {target_id}."
    ...
    audit_res = append_audit_event(
        ...
        resource=f"case:{case_id}:{target_id}",
        payload={"action_type": action_type, "parameters": parameters},
        ip_address=client_ip,
        correlation_id=correlation_id
    )

    status_str = "ACTION_CONFIRMED_AND_LOGGED" if (isinstance(req, dict) and "draft_type" in req) else "ACTION_EXECUTED"
