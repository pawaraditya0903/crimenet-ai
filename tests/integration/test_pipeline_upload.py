...
"""
    ...
    response = client.post("/api/pipeline/upload-csv", files=files, data=data, headers=headers)
...
"""
    client.post("/api/pipeline/upload-csv", files={"file": ("cdr.csv", io.BytesIO(cdr_csv.encode("utf-8")), "text/csv")}, data={"dataset_type": "cdr"}, headers=headers)
...
"""
    res = client.post("/api/pipeline/upload-csv", files={"file": ("bank.csv", io.BytesIO(bank_csv.encode("utf-8")), "text/csv")}, data={"dataset_type": "banking"}, headers=headers)
    assert res.status_code == 200
